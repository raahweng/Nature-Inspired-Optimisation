...
def fitness(screen, space, population):
    ...
    for i in range(num):
        ...
        ball.color = THECOLORS[colours[i%6]]
        ball.elasticity = 1.0
    ...
    for i in range(num):
        ...
        for j in range(99):
            ...
            line.color = THECOLORS[colours[i%6]]
...
def gene_mix(population, times):
    ...
    population = [population[i] for i in order] #no idea if this works
    new_pop = [[] for i in range(num)]
    if counter > 2: #no idea if this works
        ...
        if new_pop[i] == []:
            for j in range(100):
                ...
                if chance > 150 and j > 1 and j < 100:
                    ...
                    upper = 0
                    lower = 0
                    for k in range(1,(num-counter+1)):
                        lower = upper
                        upper += (150/((num-counter)*((num-counter)+1)/2))*k
                        if lower <= chance <= upper:
                            new_pop[i].append(population[k-1][j])
    return new_pop
...
##        population.append(points)
    ...
    while True:
        ...
        if Decimal(time.time()).quantize(Decimal('.0000000001'), rounding=ROUND_HALF_EVEN) > start+3:
            for s in space.shapes[:]:
                space.remove(s)
                try:
                    space.remove(s.body)
