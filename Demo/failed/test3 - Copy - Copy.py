...
def fitness(screen, space, population):
    ...
    for i in range(popnum):
        ...
        ball.pair_index = i+1
        ball.filter = pymunk.ShapeFilter(categories=0b1)
        space.add(body, ball) 
    ...
    for i in range(popnum):
        ...
        for j in range(pointnum-1):
            ...
            line.pair_index = i+1
            line.filter = pymunk.ShapeFilter(categories=0b1)
            space.add(line)
    ...
    for i in range(popnum):
        if new_pop[i] == []:
            ...
            for j in range(pointnum-2):
                chance = random.randint(1,pointnum)
                if chance == pointnum:
                    ...
                    for k in range(counter):
                        ...
                        if lower <= chance <= upper:
                            new_pop[i].append(population[k][j+1])
...
def main():
    ...
    body = pymunk.Body(body_type = pymunk.Body.STATIC)
    body.position = 700, 100
    line = pymunk.Segment(body, (0,0), (0,50), 1)
    line.color = THECOLORS["black"]
    line.collision_type = 1
    line.pair_index = 1000
    line.filter = pymunk.ShapeFilter(categories=0b11)
    space.add(line)
    ...
    while True:
        ...
        if Decimal(time.time()).quantize(Decimal('.0000000001'), rounding=ROUND_HALF_EVEN) > start+Decimal(1.5):
            closest = space.point_query_nearest((700,100), 100, pymunk.ShapeFilter(mask=pymunk.ShapeFilter.ALL_MASKS ^ 0b1))[0].pair_index
